...
Procedure Tratar_Lote(VAR arch_mae_act_local: archivo_mae; VAR arch_mov_local: archivo_mov; reg_mae_local: mae; reg_mov_local: mov; tipo: byte) forward;
...
Procedure LeerArchMov(var arch_mae_act_local: archivo_mae; var arch_mov_local: archivo_mov; VAR reg_local: mov);
	...
	begin
		...
		if eof(arch_mov_local) then
			begin
				//Trato el ultimo registro del archivo
				Tratar_Lote(arch_mae_act_local, arch_mov_local, reg_mae, reg_local, 2);
...
Procedure Tratar_Lote(VAR arch_mae_act_local: archivo_mae; VAR arch_mov_local: archivo_mov; reg_mae_local: mae; reg_mov_local: mov; tipo: byte);
	var
		aux_mae: mae;

	begin
		case tipo of
			1:	//Resguardo en aux el registro maestro que viene por parametro
				begin
					aux_mae := reg_mae_local;				
					//Proceso IGUALES con el registro actual luego sigo con el resto
					Iguales(aux_mae, reg_mov_local);
				end;
			2: //Creo el aux maestro (ALTA) con el primer mov
				begin
					LOG := '[ALTA] ' + IntToStr(reg_mov_local.Cuenta) + ' -- ' + reg_mov_local.Denominacion + ' -- ' + reg_mov_local.Domicilio + ' -- ' + FloatToStr(reg_mov_local.Saldo);
					WriteLog(arch_log, LOG);
		...
		while aux_mae.Cuenta = reg_mov_local.Cuenta do
			begin
				LeerArchMov(arch_mae_act_local, arch_mov_local, reg_mov_local);					
	...
	while (reg_mae.Cuenta <> HV) or (reg_mov.Cuenta <> HV) do
		begin
			...
			if reg_mae.Cuenta < reg_mov.Cuenta then
				...
				if reg_mae.Cuenta = reg_mov.Cuenta then
					begin
						Tratar_Lote(arch_mae_act, arch_mov, reg_mae, reg_mov, 1);
					...
					if reg_mae.Cuenta > reg_mov.Cuenta then
						begin
							...
							if (reg_mae.Cuenta = HV) then
								begin
									Tratar_Alta_Distintos(reg_mae, reg_mov);
									Tipo_Lote := 1;
								end;

							Tratar_Lote(arch_mae_act, arch_mov, reg_mae, reg_mov, Tipo_Lote);
